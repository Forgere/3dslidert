  ...
  function addImages(allphotos,descriptions) {
	...
	Room.prototype = {
		...
		getLayoutSettings: function () {
			...
			var perwall = Math.floor(this.totalItems / this.layout.length),
				lastwall = perwall + this.totalItems % this.layout.length;
			//墙面图片数量数组
			var wallImageNumberArray = [];
			for (var i = 0; i < this.layout.length; i++) {
				wallImageNumberArray[i] = perwall;
			}
			wallImageNumberArray[this.layout.length - 1] = lastwall;
			...
			if (dir === 'next') {
				...
				if (support.transforms3d) {
					if (this.currentItem < wallItemsCount - 1) {
						...
						// update current wall
						this.currentWall < (this.layout.length - 1) ? ++this.currentWall : this.currentWall = 0;
						// update wall
						wall = this.walls[this.currentWall];
						changeWall = true;
						// reset currentItem
				...
				if (support.transforms3d) {
					if (this.currentItem > 0) {
						...
						this.lastWall = this.currentWall;
						// update current wall
						this.currentWall > 0 ? --this.currentWall : this.currentWall = (this.layout.length - 1);
						// update wall
						wall = this.walls[this.currentWall];
						changeWall = true;
						// reset currentItem
